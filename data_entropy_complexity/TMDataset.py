...
class TMDataset:
    ...
    def __create_time_files(self):
        ...
        for current_file in filenames:
            if current_file.endswith("csv"):
                ...
                df_file = pd.read_csv(source_file_path, dtype=const.DATASET_DATA_TYPE, low_memory=False)
    ...
    def create_balanced_dataset(self, sintetic = False):
        ...
        # Modified groupby and percentage calculation
        target_df = df.groupby(['target', 'user'])['target'].count().reset_index(name='count')
        target_df['percent'] = target_df.groupby('target')['count'].transform(lambda x: 100 * x / x.sum())
        target_df['num'] = target_df.apply(lambda row: util.to_num(row, min_windows), axis=1)
        ...
        for _, row in target_df.iterrows():
            current_df = df.loc[(df['user'] == row['user']) & (df['target'] == row['target'])]
